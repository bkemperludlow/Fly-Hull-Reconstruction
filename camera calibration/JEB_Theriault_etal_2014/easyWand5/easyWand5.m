...
fprintf('\n')
disp('Welcome to easyWand version 5.7 (September 20, 2018)')
disp('Please see the associated PDF documentation for complete usage information')
disp('Visit http://www.unc.edu/~thedrick/ for updates')
fprintf('\n')
...
disp('[X   Y   Z]')
%disp(sprintf('  %.5f',reshape(tv,nCams-1,3)));
disp(num2str(squeeze(tv),4))
disp(' ')
...
if dltPass
  ...
  drmse=[];
  for i=1:numel(pdx)/2
      %Edit by Kemper Ludlow 6 August 2019: changed dlt_computeCoefficients
      %to mdlt_computeCoefficients
    [coefs(:,i), drmse(i)]=mdlt_computeCoefficients(worldXYZout,imageUV(:,i*2-1:i*2));
  end
  sbaXY_tmp=[ptMat(:,pdx);ptMat(:,pdx+size(ptMat,2)/2);ptMat2(:,pdx)];
