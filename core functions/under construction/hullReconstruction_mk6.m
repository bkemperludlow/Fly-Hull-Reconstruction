...
imageHeight = detectorLengthPix ;
imageWidth  = detectorLengthPix ;

%N         = params.N ;         % 512 ; % no. of voxels is N^3. % does not have to be the image size. can be larger.
volLength = params.volLength ; % N / 232 ; % 2.2 in cm
...
for indd= 572:(endTrackingTime-startTrackingTime+1) ; %parfor
    disp(indd) ;
    ...
    X0 = Ctr(1) - volLength/2 ; % assuming that volLength/2 is an interger number of voxelSize
    ...
    if ( double(offset(1))*voxelSize - X0 > eps || ...
        ...
        error('something is wrong with voxel or offest calcuation') ;        
    end
...
%     se = strel('disk',1,0);
%     
%     imxyWing = imopen(imxyFly-imxyBody,se);
%     imxzWing = imopen(imxzFly-imxzBody,se);
%     imyzWing = imopen(imyzFly-imyzBody,se);
    imxyWing = imxyFly - bwmorph(imxyBody,'thicken',1);
    imxzWing = imxzFly - bwmorph(imxzBody,'thicken',1);
    imyzWing = imyzFly - bwmorph(imyzBody,'thicken',1);
    ...
    goodWing    = zeros(Nx*Ny*Nz,3, 'int16');
    goodWing_counter = 0 ;
    ...
    while(queue_tail >= queue_head) % while queue is not empty        
        ...
        [isFly, isBody, isWing] = ...
            checkVoxelEverything(rcurr, dlt_xz, dlt_yz, dlt_xy, ...
                                ...
                                imxzWing, imyzWing, imxyWing, detectorLengthPix); 
...
%         end
        ...
        if (isFly)
            ...
            if (isWing)
                goodWing_counter = goodWing_counter + 1 ;
                goodWing(goodWing_counter,:) = curr ;
            end
...
%             end
    ...
    currWingCoords = goodWing(1:goodWing_counter,:) ;
    
     [largestWingCC, otherWingCC, ~, ~] = findLargestHullCC (currWingCoords);
%     %Ncc = length(CC.PixelIdxList);
%     
%     [largestWingCC2, ~, ~, ~] = findLargestHullCC (otherWingCC);
%    
     currWing1Coords = int16(largestWingCC);
     currWing2Coords = int16(otherWingCC);
...
%         disp('CC')
%     else
%         [wingClusterIdx, ctrs] = kmeans(double(currWingCoords),2,'Replicates',12);
%         wing1Idx = find(wingClusterIdx==1);
%         wing2Idx = find(wingClusterIdx==2);
%         
%         currWing1Coords = currWingCoords(wing1Idx,:);
%         currWing2Coords = currWingCoords(wing2Idx,:);
%         
%         
%         disp('kmeans')
% %         ctrDistVect1 = currWing1Coords - repmat(ctrs(1,:), size(currWing1Coords,1));
% %         ctrDist1 = sqrt( sum( ctrDistVect1 .* ctrDistVect1 ) );
% %         
% %         if (min(ctrDist1) > 1)
% %             [
% %         end
...
%     end
    ...
    if (0)
        figure(78) ; %#ok<UNRCH>
        subplot(1,3,1) ; imshow(imyz) ; hold on ; plot(center_mass_yz(1), imageHeight - center_mass_yz(2),'ro') ; title('yz') ;
        subplot(1,3,2) ; imshow(imxy) ; hold on ; plot(center_mass_xy(1), imageHeight - center_mass_xy(2),'ro') ; title('xy') ;
        subplot(1,3,3) ; imshow(imxz) ; hold on ; plot(center_mass_xz(1), imageHeight - center_mass_xz(2),'ro') ; title('xz') ;
    end
    ...
    currWing1CoordsReal = zeros(size(currWing1Coords,1),3, 'double') ;
    currWing2CoordsReal = zeros(size(currWing2Coords,1),3, 'double') ;
...
pixelxz(1) = round(pixelxz(1));
pixelxz(2) = round(detectorLengthPix-pixelxz(2)+1);
...
pixelyz(1) = round(pixelyz(1));
pixelyz(2) = round(detectorLengthPix-pixelyz(2)+1);
...
pixelxy(1) = round(pixelxy(1));
pixelxy(2) = round(detectorLengthPix-pixelxy(2)+1);
...
function [isFly, isBody, isWing] = checkVoxelEverything(r, dlt_xz, dlt_yz, dlt_xy, ...
    imxzFly, imyzFly, imxyFly, ...
    imxzBody, imyzBody, imxyBody, ...
    imxzWing, imyzWing, imxyWing, detectorLengthPix) 
...
if (imxyWing(pixelxy(2),pixelxy(1)) && ~flag)
    wingScore = wingScore + 1 ; % was 2 before (TB)
end

 if (imxzWing(pixelxz(2),pixelxz(1))) %&& ~flag)
     wingScore = wingScore + 1 ;
end
 
if (imyzWing(pixelyz(2),pixelyz(1))) %&& ~flag)
    wingScore = wingScore + 1 ;
end

if (wingScore >= 3) % was 2 before (TB)
    isWing = true;
    return
end
