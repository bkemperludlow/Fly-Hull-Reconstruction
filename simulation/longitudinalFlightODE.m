...
% -------------------------------------------------------------------------
function ds = longitudinalFlightODE(t, s, Z, params, thetaB0, pertFlag)
%----------------------------
%% initialize parameters
% booleans for added mass force, smoothing, etc
addedMassFlag = false ; 

% degree to radian conversion in params struct wing kin values 
deg2RadFlag = true ; 
...
delta_phi_f = pitch_controller_func(thetaB_lag, thetaB_dot_lag, thetaB0,...
    K_i, K_p) ;

wing_kin_params = read_wing_kin_params(params, deg2RadFlag, delta_phi_f) ;
...
% tic
[F_tot, T_tot] = fakeWingKinForceAndTorque(wing_kin_params, body_params,...
    t, velBodyBody, bodyYPR, bodyYPR_dot, params, addedMassFlag, false) ;
% toc
% -------------------------------------------------------------------
...
function wing_kin_params = ...
    read_wing_kin_params(params, deg2RadFlag, delta_phi_f)
% -----------------------------------------
% inputs and params
if ~exist('deg2RadFlag','var') || isempty(deg2RadFlag)
    deg2RadFlag = false ; 
end
if ~exist('delta_phi_f','var') || isempty(delta_phi_f)
    delta_phi_f = 0 ; 
end
