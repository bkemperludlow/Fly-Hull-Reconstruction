function ds = bodyPitchODE_CTRL(t,s,Z,K_i,K_p,...
...
% Read state vector elements into variables (for simplicity)
theta = s(5) ; %body pitch angle
thetadot = s(6) ; %rotational velocity
thetaLag = Z(5) ; %pitch angle at theta(t - \DeltaT)
thetaDotLag = Z(6) ; 
...
%Right wing:
phiR = phi_func(t,thetaLag,thetaDotLag) ;
%thetaL = 0 ;
psiR = psi_func(t) ; 

%Left wing:
phiL = phi_func(t,thetaLag,thetaDotLag) ;
%thetaL = 0 ;
psiL = psi_func(t) ;
...
C_L_R = 1.2*sin(2*(pi/180)*alphaR) ; %.225 + 1.58*sin((pi/180)*(2.13*alphaR-7.2)) ;
C_D_R = 1.4 - cos(2*(pi/180)*alphaR) ;%1.92 - 1.55*cos((pi/180)*(2.04*alphaR-9.82)) ;
C_L_L = 1.2*sin((pi/180)*alphaL) ; %.225 + 1.58*sin((pi/180)*(2.13*alphaL-7.2)) ;
C_D_L = 1.4 - cos(2*(pi/180)*alphaL) ;%1.92 - 1.55*cos((pi/180)*(2.04*alphaL-9.82)) ;
...
%adds a perturbing torque like the one from the Helmholtz coils
if (pertFlag) && (t > pulseStart) && (t < pulseEnd)
    ds(6) = ds(6) + pulseStrength ; 
end
...
%}
%% Misc functions
function phi = phi_func(t, thetaLag, thetaDotLag)
    if t > .350
        Dphi_front = (180/pi)*(K_i*(thetaLag-beta_0) + K_p*thetaDotLag) ;
    else
        Dphi_front = (180/pi)*(.3*(thetaLag-beta_0) + .007*thetaDotLag) ;
    end
    if Dphi_front > 50
        Dphi_front = 50 ;
    elseif Dphi_front < -50
        Dphi_front = -50 ;
    end    
    phi_0_prime = phi_0 + Dphi_front/2 ;
    phi_m_prime = phi_m - Dphi_front/2 ;
    phi = phi_0_prime + phi_m_prime*asin(K*sin(omega*t))/asin(K) ;
