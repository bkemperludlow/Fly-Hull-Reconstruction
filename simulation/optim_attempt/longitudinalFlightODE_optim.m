...
% -------------------------------------------------------------------------
function [ds, delta_phi_f] = longitudinalFlightODE_optim(t, s, Z, ...
...
%     thetaB_dot_lag = Z(6,:) ; % LAGGED body pitch velocity
    thetaB_lag = mean(Z(5,:)) ;     % LAGGED body pitch angle
    thetaB_dot_lag = mean(Z(6,:)) ; % LAGGED body pitch velocity
else
...
% if t > 0.45
%    t_phase = wrapTo2Pi(rem(params.omega*t, 2*pi)) ; 
%    fprintf('t = %f \n',t)
%    fprintf('phase = %f \n', t_phase)
%    fprintf('delta_phi_f = %f \n', delta_phi_f)
%    fprintf('pitch torque (wing) = %f \n', (1/Iyy)*(-T_tot(2)))
%    fprintf('pitch torque (total) = %f \n', ds(6))
%    keyboard
% end
