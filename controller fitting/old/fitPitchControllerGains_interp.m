function [K_i, K_p, K, resnorm, residual, J] = ...
    fitPitchControllerGains_interp(fwdFlipTimes, deltaPhiFront, deltaT, c_pitch, paramGuess, plotFlag) 

%x = [ K_i, K_p, K] 

t_range = fwdFlipTimes(1): 0.125e-3 : fwdFlipTimes(end) ; 
bodyPitch = c_pitch(t_range - deltaT) ; 
theta_0 = c_pitch(0) ;
pitchVelocity = differentiate(c_pitch, t_range - deltaT) ;
deltaBodyPitch = bodyPitch - theta_0 ; 
bodyData = [deltaBodyPitch' ; pitchVelocity'] ; 

c_deltaPhiFront = fit(fwdFlipTimes', deltaPhiFront','cubicinterp') ; 
deltaPhiFront_interp = c_deltaPhiFront(t_range) ; 
...
[x, resnorm, residual, ~, ~, ~, J] = ...
    lsqcurvefit(controllerEq, paramGuess, bodyData,deltaPhiFront_interp',lb,ub,options) ;
