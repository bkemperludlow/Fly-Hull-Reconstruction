function [anglesLabFrame, anglesBodyFrame, t, newEtaLab, newEtaBody, sp_rho, smoothed_rho, rho_t, rho_samp] = ...
    calcAngles_quick_and_dirty_mk2(data, rollErr, plotFlag,bigPitchFlag)
%
...
%test edit by SW 5/2/2016
if bigPitchFlag
    rawPsi = (180/pi)*unwrap((pi/180)*rawPsi) ;
    bodyCM = data.bodyCM ;
    d1 = designfilt('lowpassiir','FilterOrder',8,'SampleRate',8000, ...
        'HalfPowerFrequency',100,'DesignMethod','butter');
    bodyCM_filt = filtfilt(d1,bodyCM) ;
    bodyVel = diff(bodyCM_filt,1) ;
    bodyVel_xy = [bodyVel(:,1:2), zeros(size(bodyVel(:,3)))] ;
    bodyVel_xy_hat = bodyVel_xy./repmat(myNorm(bodyVel_xy),1,3) ;
    bodyVel_xy_hat = [bodyVel_xy_hat(1,:) ; bodyVel_xy_hat] ;
    AHat = data.AHat ;
    pitchAxis = cross(bodyVel_xy_hat, repmat([0 0 1],size(bodyVel_xy_hat,1),1),2) ;
    AHat_proj = AHat - pitchAxis.*repmat(dot(pitchAxis,AHat,2),1,3) ;
    AHat_proj = AHat_proj./repmat(myNorm(AHat_proj),1,3) ;
    
    rawBeta = (180/pi)*acos(dot(AHat_proj,bodyVel_xy_hat,2)) ;
end
...
if rhoFlag
    ...
    if binRhoTimesFlag
       ...
       rollVectors = nan(data.Nimages,3) ; 
...
for k=1:data.Nimages
    ...
    phiRdeg   = unwrap(atan2(rotVeinR(2),rotVeinR(1))) * C;
    ...
    % left wing
    % ---------
    phiLdeg   = unwrap(atan2(rotVeinL(2),rotVeinL(1))) * C;
