...
rotM_init = eulerRotationMatrix(deg2rad*rawPsi(1), deg2rad*rawBeta(1),0) ; 
...
% now loop through frames and try to rotate everybody onto x axis just
...
for i = 2:N_frames
    ...
    bodyYaw_rot(i) = rad2deg*dYaw ;
    bodyPitch_rot(i) = rad2deg*dPitch ; 
    
    rotM_array(:,:,i) = rotM_curr*rotM_prev ; 
end

bodyPitch = cumsum(bodyPitch_rot) ; 
bodyYaw = cumsum(bodyYaw_rot) ; 
