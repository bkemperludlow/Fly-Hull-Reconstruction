...
function [flyGrp, bodyGrp, rightWingGrp, leftWingGrp, dL, rightWingTip, leftWingTip ] = ....
    draw3Dfly_v2(ax, scale, resolution, pinType, thetab0) %#ok<INUSD>
% this version uses two meshes for the fly body:
% a fine mesh for the surface and a coarse mesh (20) for the grid
% this comes to solve a rendering problem with opengl (need for
% transparency) and to make the surface lighting look like phong.
...
leftWingColor  = [194 97 20]/255*.6 ;%[0.5 0.5 1 ] ; % [0 0 1]
...
if gridFlag
    ...
    hold on ;
    for k=0:9
        hcir = plot3(xc,yc,zc,'k-','linewidth',0.5) ;
